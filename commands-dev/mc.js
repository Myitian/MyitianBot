const { SlashCommandBuilder, CommandInteractionOptionResolver, EmbedBuilder, AttachmentBuilder, escapeMarkdown } = require("discord.js");
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName("skyblock")
                .setDescription("Hypixel Skyblock")
                .addStringOption(option =>
                    option.setName("name")
                        .setDescription("玩家名")
                        .setRequired(true))),
            ...
            case "ping":
                {
                    ...
                    try {
                        ...
                        embeds.push(new EmbedBuilder()
                            .setTitle(escapeMarkdown(text_component.asString(info.version.name)))
                            .setThumbnail("attachment://favicon.png")
                            .setDescription(escapeMarkdown(text_component.asString(info.description)))
                            .addFields(
                                { name: "协议版本", value: info.version.protocol.toString(), inline: true },
                                { name: "玩家", value: `${info.players.online}/${info.players.max}`, inline: true },
                                { name: "地址", value: escapeMarkdown((rawPort === null || rawPort === undefined || Number.isNaN(rawPort)) ? rawHost : `${rawHost}:${rawPort}`), inline: true }
                            ));
                    } catch (e) {
                        content = "无法解析或连接服务器";
            ...
            case "skin":
                {
                    ...
                    if (data.id) {
                        embeds.push(new EmbedBuilder()
                            .setTitle(escapeMarkdown(data.name))
